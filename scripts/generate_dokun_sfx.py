"""Generate the vertical ドクンッ！ brush-style SFX mask."""
...
OUTPUT = ROOT / "web" / "assets" / "sfx" / "dokun-small-tsu-mask.webp"
...
def main() -> None:
    ...
    chars = list("ドクンッ！")
    factors = [1.22, 1.10, 0.98, 0.84, 0.72]
    gap = 7
    usable_height = 432 - gap * (len(chars) - 1)
    base_height = usable_height / sum(factors)
    rng = random.Random("dokun-small-tsu-mask")
    ...
    for index, (char, factor) in enumerate(zip(chars, factors)):
        ...
        target_w = max(24, round(target_h * aspect * (1.10 - index * 0.055)))
